      ...
      <div className="max-w-4xl mx-auto">
        <div className="grid lg:grid-cols-3 gap-12">
          <div className="lg:col-span-1">
            <h2 className="text-3xl font-title-light mb-8 tracking-wide">Experience</h2>
          </div>
            ...
            {experiences.map((exp, index) => (
              <div key={index} className="space-y-6">
                <div className="space-y-3">
                  ...
                  <h3 className="text-xl font-title-medium leading-tight">{exp.title}</h3>
                  <p className="text-lg font-body text-foreground">{exp.company}</p>
                  <p className="text-sm text-muted-foreground font-mono">{exp.location}</p>
                </div>

                <ul className="space-y-3">
                  {exp.description.map((item, i) => (
                    <li key={i} className="text-muted-foreground leading-relaxed font-body relative pl-4">
